...
class Var(object):
    ...
    if 'DYNO' in environ:
        ...
        ON_HEROKU = False
    FQDN = str(getenv('FQDN', 'BIND_ADRESS:PORT')) if not ON_HEROKU or getenv('FQDN', '159.223.40.206') else APP_NAME+'.herokuapp.com'
    HAS_SSL=bool(getenv('HAS_SSL',True))
    if HAS_SSL:
        URL = "http://{159.223.40.206:8080}/".format(FQDN)
    else:
        URL = "http://{159.223.40.206:8080}/".format(FQDN)
